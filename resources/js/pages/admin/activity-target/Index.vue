...
const deleteItem = (row) => handleDelete({
  message: `Hapus Target Kegiatan ${row.type.name} tanggal ${dayjs(row.date).format('DD MMMM YYYY')}?`,
...
const onRowClicked = (row) => router.get(route("admin.activity-target.detail", { id: row.id }));
...
<template>
  ...
  <authenticated-layout>
    ...
    <template #right-button>
      <q-btn icon="add" dense color="primary" @click="router.get(route('admin.activity-target.add'))" />
        ...
        <q-menu anchor="bottom right" self="top right" transition-show="scale" transition-hide="scale">
          <q-list style="width: 200px">
            <q-item clickable v-ripple v-close-popup
              :href="route('admin.activity-target.export', { format: 'pdf', filter: filter })">
            ...
            <q-item clickable v-ripple v-close-popup
              :href="route('admin.activity-target.export', { format: 'excel', filter: filter })">
              <q-item-section avatar>
                  ...
                  <q-menu anchor="bottom right" self="top right" transition-show="scale" transition-hide="scale">
                    <q-list style="width: 200px">
                      ...
                      <q-item v-if="['bs', 'admin'].includes($page.props.auth.user.role)" clickable v-ripple
                        v-close-popup @click.stop="router.get(route('admin.activity-target.duplicate', props.row.id))">
                        <q-item-section avatar>
                      ...
                      <q-item v-if="['bs', 'admin'].includes($page.props.auth.user.role)" clickable v-ripple
                        v-close-popup @click.stop="router.get(route('admin.activity-target.edit', props.row.id))">
                        <q-item-section avatar>
                          <q-icon name="edit" />
                        </q-item-section>
